...
class HealthAnalyzer:
    ...
    def calculate_health_score(self, package_info: Dict, repo_info: Dict) -> int:
        ...
        # ---- 1. Set higher baseline scores based on update recency ----
        min_baseline_score = 30  # Default minimum score for any package (increased from 0)
        if package_info.get("lastPublished"):
            try:
                ...
                # Set baseline scores for recently updated packages
                if days_since_publish < 90:  # Last 3 months
                    min_baseline_score = 45  # Increased from 20
                elif days_since_publish < 180:  # Last 6 months
                    min_baseline_score = 40  # Increased from 15
                elif days_since_publish < 365:  # Last year
                    min_baseline_score = 35  # Increased from 10
                elif days_since_publish < 730:  # Last 2 years
                    min_baseline_score = 30  # New threshold
            except Exception:
                # If date parsing fails, use default minimum baseline
                pass
        
        # ---- 2. Score based on update frequency (keep at 30 points) ----
        if package_info.get("lastPublished"):
            try:
                from dateutil import parser
                last_published = parser.parse(package_info["lastPublished"])
                ...
                if days_since_publish < thresholds[0]:
                    ...
                    score += weight * 0.85  # Increased from 0.8
                elif days_since_publish < thresholds[2]:
                    score += weight * 0.7   # Increased from 0.6
                elif days_since_publish < thresholds[3]:
                    score += weight * 0.5   # Increased from 0.3
                else:
                    score += weight * 0.3   # Increased from 0.1
            except Exception:
                # If date parsing fails, give a moderate score for this metric
                score += self.metrics["update_frequency"]["weight"] * 0.3  # Increased from 0.1
        
        # ---- 3. Score based on GitHub metrics ----
        github_score = 0
        github_weight_total = 0
        
        # Use a fallback mechanism - either GitHub metrics or alternative scoring
        if repo_info:
            # Stars
            stars = repo_info.get("stars", 0)
            stars_weight = self.metrics["stars"]["weight"]
            github_weight_total += stars_weight
            thresholds = self.metrics["stars"]["thresholds"]
            
            if stars >= thresholds[4]:
                github_score += stars_weight
            elif stars >= thresholds[3]:
                github_score += stars_weight * 0.85  # Increased from 0.8
            elif stars >= thresholds[2]:
                github_score += stars_weight * 0.7   # Increased from 0.6
            elif stars >= thresholds[1]:
                github_score += stars_weight * 0.5   # Increased from 0.4
            elif stars >= thresholds[0]:
                github_score += stars_weight * 0.3   # Increased from 0.2
            
            # Forks
            forks = repo_info.get("forks", 0)
            forks_weight = self.metrics["forks"]["weight"]
            github_weight_total += forks_weight
            thresholds = self.metrics["forks"]["thresholds"]
            
            if forks >= thresholds[4]:
                github_score += forks_weight
            elif forks >= thresholds[3]:
                github_score += forks_weight * 0.85  # Increased from 0.8
            elif forks >= thresholds[2]:
                github_score += forks_weight * 0.7   # Increased from 0.6
            elif forks >= thresholds[1]:
                github_score += forks_weight * 0.5   # Increased from 0.4
            elif forks >= thresholds[0]:
                github_score += forks_weight * 0.3   # Increased from 0.2
            
            # Contributors
            contributors = repo_info.get("contributorsCount", 0)
            contributors_weight = self.metrics["contributors"]["weight"]
            github_weight_total += contributors_weight
            thresholds = self.metrics["contributors"]["thresholds"]
            
            if contributors >= thresholds[4]:
                github_score += contributors_weight
            elif contributors >= thresholds[3]:
                github_score += contributors_weight * 0.85  # Increased from 0.8
            elif contributors >= thresholds[2]:
                github_score += contributors_weight * 0.7   # Increased from 0.6
            elif contributors >= thresholds[1]:
                github_score += contributors_weight * 0.5   # Increased from 0.4
            elif contributors >= thresholds[0]:
                github_score += contributors_weight * 0.3   # Increased from 0.2
            
            # Commit frequency (enhanced)
            commit_freq = repo_info.get("commitFrequency", 0)
            if commit_freq > 10:  # More than 10 commits per week
                github_score += 10  # Increased from 5
            elif commit_freq > 5:  # More than 5 commits per week
                github_score += 7   # Increased from 3
            elif commit_freq > 1:   # More than 1 commit per week
                github_score += 4   # Increased from 1
            elif commit_freq > 0:   # Any activity
                github_score += 2   # New threshold
        ...
        # ---- 4. Alternative scoring for non-GitHub or missing repo info ----
        # If we don't have GitHub info or the GitHub score is very low, use alternative metrics
        if not repo_info or github_score < 15:  # Increased threshold from 10
            # Maintainers score - enhanced for packages without GitHub
            maintainers = package_info.get("maintainers", 0)
            maintainer_score = 0
            if maintainers > 5:
                maintainer_score = 25  # Increased from 15
            elif maintainers > 2:
                maintainer_score = 20  # Increased from 10
            elif maintainers > 0:
                maintainer_score = 15  # Increased from 5
                
            # Only add maintainer score if we didn't use GitHub metrics (to avoid double counting)
            if not repo_info:
                score += maintainer_score
            else:
                # If we have both, take the better score
                score += max(0, maintainer_score - github_score)
            
            # Package ecosystem significance - estimate based on the ecosystem and package name
            ecosystem = package_info.get("ecosystem", "").lower()
            package_name = package_info.get("name", "").lower() if package_info.get("name") else ""
            
            # Expanded list of major packages
            major_python_packages = [
                "numpy", "pandas", "flask", "django", "requests", "scipy", "pillow", 
                "tensorflow", "scikit-learn", "matplotlib", "beautifulsoup4", "werkzeug",
                "pytest", "sqlalchemy", "jinja2", "celery", "redis", "flask-login",
                "flask-sqlalchemy", "gunicorn", "virtualenv", "click", "pyyaml", "setuptools",
                "pip", "wheel", "twine", "black", "pylint", "mypy", "opencv-python", "torch",
                "protobuf", "lxml", "psycopg2", "pymysql", "cx_oracle", "anthropic"
            ]
            
            major_npm_packages = [
                "react", "vue", "angular", "express", "lodash", "moment", "jquery", 
                "tailwindcss", "bootstrap", "redux", "axios", "typescript", "next",
                "webpack", "babel", "eslint", "prettier", "jest", "cypress", "mocha",
                "chai", "storybook", "material-ui", "chakra-ui", "styled-components",
                "emotion", "node-fetch", "react-dom", "react-router", "graphql", "apollo-client",
                "formik", "react-query", "swr", "vite", "alpinejs", "chart.js"
            ]
            
            # Apply score boost for known major packages
            if ecosystem == "pypi" and package_name in major_python_packages:
                score += 25  # Increased from 15
            elif ecosystem == "npm" and package_name in major_npm_packages:
                score += 25  # Increased from 15
        
        # ---- 5. Version compatibility score ----
        # Check if latest version matches current version
        if package_info.get("latestVersion") == package_info.get("version"):
            score += 10  # Increased from 5 - Reward packages that are up-to-date
        
        # ---- 6. Apply minimum baseline score ----
        score = max(score, min_baseline_score)
    ...
    def _analyze_dependencies(self) -> pd.DataFrame:
        ...
        for _, row in df.iterrows():
            ...
            if has_vulnerabilities:
                # Extract vulnerability details for report
                for vuln in vulnerabilities:
                ...
                if latest_version and latest_version != current_version:
                    safe_version = self.vulnerability_scanner.get_safe_upgrade_version(
                        ecosystem, package_name, current_version, latest_version
                    )
